...
import type { ExperienceItem } from "@/types/locale";
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 px-0 md:px-4">
          ...
          <div className="grid lg:grid-cols-12 gap-6 lg:gap-10">
            <div className="lg:col-span-4">
              <h2 className="text-xl md:text-3xl lg:text-2xl font-bold text-text-primary leading-tight">
                <span className="text-gold italic">{t("experience.titleHighlight")}</span>{" "}
                {t("experience.title")}
              </h2>
            </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 lg:gap-10">
          <div
            className={`lg:col-span-4 transition-all duration-1000 delay-200 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}
          >
            <div className="space-y-1">
              {experiences.map((exp, index) => (
                ...
                >
                  ...
                  <p className="text-text-muted text-[10px] md:text-xs mt-1">{exp.period}</p>
                  <p className="text-text-secondary text-[10px] md:text-xs mt-0.5">
                    {getLocalizedText(exp.role, locale)}
                  </p>
                </button>
              ))}
            </div>
          ...
          >
            <div className="mb-4">
              <h3 className="text-text-primary text-xs md:text-sm font-semibold">
                {t("experience.showcase")}{" "}
                <span className="text-gold">{selectedExperience.company}</span>
              </h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div className="group relative aspect-video md:aspect-[4/5] rounded-xl overflow-hidden bg-dark-card border border-purple-deep/30 hover:border-purple-glow/50 transition-all duration-300">
                <div className="absolute inset-0 bg-gradient-to-br from-purple-deep/30 to-dark-surface flex items-center justify-center">
                  {selectedExperience.showcase[0]?.image ? (
                    <Image
                      src={selectedExperience.showcase[0].image}
                      alt={getLocalizedText(selectedExperience.showcase[0].title, locale)}
                      fill
                    ...
                    <div className="text-center p-4">
                      <div className="w-12 h-12 mx-auto mb-2 rounded-lg bg-purple-glow/20 flex items-center justify-center">
                        <svg
                          className="w-6 h-6 text-purple-light"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                      ...
                      <span className="text-text-muted text-[10px]">
                        {getLocalizedText(selectedExperience.showcase[0]?.title, locale)}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-3 md:gap-4">
                {selectedExperience.showcase.slice(1, 3).map((project) => (
                  <div
                    key={project.id}
