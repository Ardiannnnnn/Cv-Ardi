          ...
          <div className="relative w-full h-full bg-dark-surface/90 backdrop-blur-xl rounded-2xl border border-purple-glow/30 overflow-hidden shadow-2xl transform rotate-y-[-5deg] rotate-x-[5deg]">
            ...
            <div className="p-4 font-mono text-xs md:text-sm space-y-1 overflow-hidden">
              ...
              <div className="flex">
                ...
                <span className="text-green-400 ml-2">&quot;{role}&quot;</span>
                <span className="text-text-muted">,</span>
              </div>
              <div className="flex">
...
export default function About() {
  ...
  const handleDownloadCV = () => {
    ...
    link.href = personal.cv.path;
    link.download = personal.cv.filename;
  ...
  const university = t("personal.university");
      ...
      >
        ...
        <div className="max-w-7xl mx-auto w-full relative z-10 px-0 md:px-4">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            >
              <div className="mb-2">
                <h2 className="text-xl md:text-3xl lg:text-2xl font-bold text-text-primary leading-tight">
                  <span className="text-gold">{t("personal.degreeHighlight")}</span>{" "}
                  {t("personal.degree")}
                  <br />
                </h2>
              </div>
              ...
              <div className="flex flex-wrap gap-6 mb-6">
                <div>
                  <h4 className="text-text-primary font-semibold text-xs md:text-sm">
                    {t("personal.program")}
                  </h4>
                  <p className="text-text-muted text-[10px] md:text-xs">{university}</p>
                </div>
                <div>
                  <h4 className="text-text-primary font-semibold text-xs md:text-sm">
                    {t("personal.role")}
              ...
              <div className="space-y-3 mb-6">
                <p className="text-text-secondary text-xs md:text-sm leading-relaxed">
                  {t("about.bio1", { name: personal.name, university })}
                </p>
            ...
            >
              <Floating3DElement name={personal.name} role={t("personal.role")} />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="relative w-full max-w-4xl h-[90vh] bg-dark-surface rounded-xl shadow-2xl overflow-hidden border border-purple-glow/30">
            <div className="flex justify-between items-center p-4 bg-dark-card border-b border-purple-deep/30">
              <h3 className="text-text-primary text-xl font-semibold">
                {t("about.cvPreviewTitle", { name: personal.name })}
              </h3>
            ...
            <div className="w-full h-[calc(100%-4rem)]">
              <iframe
                src={`${personal.cv.path}#toolbar=1&navpanes=0&scrollbar=1`}
                className="w-full h-full"
                title={t("about.previewCv")}
              />
            </div>
          </div>
        </div>
