...
export default function Home() {
  ...
  const maxSlide = Math.max(0, projectsData.length - projectsPerSlide);
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto">
        <div className="relative flex flex-col items-center justify-center min-h-[85vh] md:min-h-[80vh]">
          <div
            className={`absolute top-6 sm:top-10 md:top-4 left-1/2 -translate-x-1/2 transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-10"}`}
          >
          ...
          <div
            className={`absolute top-0 md:top-4 right-0 hidden md:flex flex-col items-end gap-1.5 transition-all duration-1000 delay-300 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`}
          >
          ...
          <div className=" relative flex items-center justify-center w-full -mt-8 sm:-mt-4 md:mt-0">
            <div
              className={`relative z-10 transition-all duration-1000 delay-500 ${isVisible ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
            >
          ...
          <div className="absolute bottom-4 md:bottom-0 left-0 right-0 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-8 px-0 md:px-4">
            <div
              className={`max-w-[280px] md:max-w-xs transition-all duration-1000 delay-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
            >
              <h3 className="text-text-primary font-semibold text-base md:text-lg mb-2 md:mb-3">
                {t("home.aboutTitle")}
              </h3>
              <p className="text-text-secondary text-xs md:text-sm leading-relaxed">
                {t("home.aboutTextPrefix")}{" "}
                <span className="text-gold font-medium">{personal.name}</span>.{" "}
                {t("home.aboutTextSuffix")}
              </p>
              <button
              ...
              >
                <ArrowDownIcon />
                <span className="group-hover:underline">{t("home.moreInfo")}</span>
              </button>
            </div>

            <div
              className={`hidden md:block transition-all duration-1000 delay-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
            >
              <h3 className="text-text-primary font-semibold text-lg mb-3 text-right">
                {t("home.lastProject")}
              </h3>
          ...
          <div
            className={`md:hidden absolute bottom-[-40px] left-0 right-0 flex justify-center gap-4 transition-all duration-1000 delay-500 ${isVisible ? "opacity-100" : "opacity-0"}`}
          >
